...
def startCalculation(inputData):
  ...
  if lines > 0:
    ...
    filename = "../results/{directory}{filename}".format(filename = os.path.basename(item), directory = inputData["directory"])
  ...
  analyzer = subprocess.Popen(["../cpp/analyzer/linearizer", "invocation", str(lines), item], stdout=resultFile)
  analyzer.wait()
  # resultFile.close()
...
def calcOpFairness(directory):
  ...
  outputDir = "../results/{directory}".format(directory = directory)
